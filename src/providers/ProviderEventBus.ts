...
export interface IngestionResult {
  accepted: boolean;
  reason?: 'DUPLICATE' | 'OUT_OF_ORDER' | 'INVALID_TRANSITION' | 'CALL_NOT_FOUND' | 'APPLIED';
}
...
export class ProviderEventBus {
   ...
   * 2. Sequence Check (monotonic sequenceNumber)
   * 3. State Transition Validation via Call State Machine
   * 4. Agent Side-effect updates
   */
  processEvent(event: ProviderEvent): IngestionResult {
    // 1. Idempotency Check: Drop if already processed
    const now = Date.now();
    ...
    // 6. Handle Agent State Transitions on Call State Changes
    if (call.agentId) {
